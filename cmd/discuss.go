...
func newDiscussCommand() *cobra.Command {
	...
	promote.Flags().BoolVar(&promoteApply, "apply", false, "create the promoted issue set after review for supported backends")
	promote.Flags().BoolVar(&promoteConfirm, "confirm", false, "required acknowledgement before apply mutates the selected backend")
